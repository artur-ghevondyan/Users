...
@AndroidEntryPoint
class UserFragmentWithLessThreeRepo: BaseFragment<FragmentUserBinding>() {
    ...
    private val args: UserFragmentWithLessThreeRepoArgs by navArgs()

    override fun prepareView(savedInstanceState: Bundle?) {
        if (viewModel.currentState.userState is UserContract.UserState.Idle)
            viewModel.setEvent(UserContract.Event.OnFetchUser(args.userName))

        binding.txtMoreThenThree.text = getString(R.string.less_than_three_repo)
